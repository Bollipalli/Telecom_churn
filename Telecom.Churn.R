getwd()
setwd("C:/Jig15899")
tele<-read.csv("file:///C:/Data Science with R/Assignments/Graded Assignments/Topic 13 -  Final Case Study Course Wrap up/telecomfinal.csv", stringsAsFactors = T)
tele
library(dplyr)
names(tele)
str(tele)
...
dqr$No.ofRecords
#counting of unique values for each variable
...
dqr$MissingPercentage
#Minimum, Maximum, Mean, Quantile Values for each Variable
{
  dqr$Minimum[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,1])=="numeric",min(tele[,1],na.rm = T),0),2)
  dqr$Maximum[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,1])=="numeric",max(tele[,1],na.rm = T),0),2)
  dqr$mean[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,1])=="numeric",mean(tele[,1],na.rm = T),0),2)
  dqr$fifthPercentile[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,i])=="numeric",quantile(tele[,i],p=0.05,na.rm = T),0),2)
  dqr$tenthPercentile[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,i])=="numeric",quantile(tele[,i],p=0.10,na.rm = T),0),2)
  dqr$twentyfifthPercentile[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,i])=="numeric",quantile(tele[,i],p=0.25,na.rm = T),0),2)
  dqr$fiftythPercentile[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,i])=="numeric",quantile(tele[,i],p=0.50,na.rm = T),0),2)
  dqr$seventyfifthPercentile[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,i])=="numeric",quantile(tele[,i],p=0.75,na.rm = T),0),2)
  dqr$ninetythPercentile[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,i])=="numeric",quantile(tele[,i],p=0.90,na.rm = T),0),2)
  dqr$ninetyfifthPercentile[i]<-round(ifelse(class(tele[,1])=="integer"|class(tele[,i])=="numeric",quantile(tele[,i],p=0.95,na.rm = T),0),2)
}
str(dqr)
#Exporting Data Quality Report
write.csv(dqr,"Data Quality Report.csv", row.names = T)

#Missing Value treament of var, "retdays" and Creating Dummy Variables
...
#Variable drop_blk_Mean is created by adding vars blck_dat_Mean +BLK_VCE_MEAN + DROP_DAT_MEAN + DROP_VCE_MEAN
#so omitting Variable blck_dat_Mean
names(tele1)
tele1<-tele1[,-50]

##Data Exploration -> profiling (dat-continues variables, datC-Categorical variables)*********
names(tele1)
str(tele1)
...
dat2$varnames<-rep("totmrc_Mean",nrow(dat2))
...
dat4$varnames<-rep("mou_Range",nrow(dat4))
...
dat5$varnames<-rep("change_mou",nrow(dat5))
...
dat6$varnames<-rep("drop_blk_Mean",nrow(dat6))
...
dat7$varnames<-rep("drop_vce_Range",nrow(dat7))
...
dat8$N<-unclass(tele1%>%mutate(dec=ntile(drop_blk_Mean,n=10))%>%count(dec)%>%unname())[[2]]
...
dat8$LessThan<-unclass(tele1%>%mutate(dec=ntile(drop_blk_Mean,n=10))%>%group_by(dec)%>%summarise(max(owylis_vce_Range)))[[2]]
dat8$varnames<-rep("owylis_vce_Range",nrow(dat8))
...
dat9$varnames<-rep("mou_opkv_Range",nrow(dat9))
...
dat11$varnames<-rep("totcalls",nrow(dat11))
...
dat12$varnames<-rep("eqpdays",nrow(dat12))

#<13> Variable "custcare_Mean"===> ***Getting less than 4deciles. Omit****
...
dat14$varnames<-rep("callwait_Mean",nrow(dat14))
...
dat15$varnames<-rep("iwylis_vce_Mean",nrow(dat15))

#<16> Variable "callwait_Rang"====> ****Getting less than 4 deciles. Omit****
...
dat18$varnames<-rep("adjqty",nrow(dat18))
...
dat19$varnames<-rep("ovrrev_Mean",nrow(dat19))
...
dat20$LessThan<-unclass(tele1%>%mutate(dec=ntile(ovrrev_Mean,n=10))%>%group_by(dec)%>%summarise(max(rev_Mean)))[[2]]
dat20$varnames<-rep("rev_Mean",nrow(dat20))
...
dat21$varnames<-rep("ovrmou_Mean",nrow(dat21))
...
dat22$varnames<-rep("ovrrev_Mean",nrow(dat22))
...
dat23$varnames<-rep("plcd_vce_Mean",nrow(dat23))
...
dat24$varnames<-rep("avg3mou",nrow(dat24))
...
dat25$varnames<-rep("avgmou",nrow(dat25))
...
dat26$varnames<-rep("avg3qty",nrow(dat26))

#<27> Variable "avgqty"
summary(tele1$avgqty)
tele1%>%mutate(dec=ntile(avgqty,n=10))%>%count(churn,dec)%>%filter(churn==1)->dat26
dat26$N<-unclass(tele1%>%mutate(dec=ntile(avgqty,n=10))%>%count(dec)%>%unname())[[2]]
dat26$churn_perc<-dat26$n/dat26$N
dat26$GreaterThan<-unclass(tele1%>%mutate(dec=ntile(avgqty,n=10))%>%group_by(dec)%>%summarise(min(avgqty)))[[2]]
dat26$LessThan<-unclass(tele1%>%mutate(dec=ntile(avgqty,n=10))%>%group_by(dec)%>%summarise(max(avgqty)))[[2]]
dat26$varnames<-rep("avgqty",nrow(dat26))
...
dat28$varnames<-rep("avg6mou",nrow(dat28))
...
dat29$varnames<-rep("avg6qty",nrow(dat29))
...
dat30$varnames<-rep("age1",nrow(dat30))

#<31> Variable "age2" ===> ***Getting less than 4 delices.use as factor***
summary(tele1$age2)
...
dat33$N<-unclass(tele1%>%mutate(dec=ntile(age1,n=10))%>%count(dec)%>%unname())[[2]]
...
dat33$varnames<-rep("hnd_price",nrow(dat33))
...
dat34$varname<-rep("actvsub",nrow(dat34))
...
#<36> Variable "forgntv1" ====> ***Factor Variable****
summary(tele1$forgntvl)
tele1%>%mutate(dec=ntile(forgntvl,n=1))%>%count(churn,dec)%>%filter(churn==1)->dat36
dat36$varname<-rep("forgntv1",nrow(dat36))

#<37> Variable "opk_dat_Mean" ====>***Omit***
summary(tele1$opk_dat_Mean)
tele1%>%mutate(dec=ntile(opk_dat_Mean,Mean=4))%>%count(churn,dec)%>%filter(churn==1)->dat37
...
#<41> Variable "recv_sms_Mean" ===> ***Getting less than4 deciles. so Omit***
...
#<42> Variable "mou_pead_Mean" ===> ***Getting less than 4 delices. So Omit***
...
dat43$varnames<-rep("da_Mean",nrow(dat43))
...
dat44$varnames<-rep("da_Range",nrow(dat44))
...
dat48$varnames<-rep("drop_vce_Mean",nrow(dat48))
...
dat49$GreaterThan<-unclass(tele1%>%mutate(dec=ntile(49,n=10))%>%group_by(dec)%>%summarise(min(adjmou)))[[2]]
dat49$LessThan<-unclass(tele1%>%mutate(dec=ntile(adjmou,n=10))%>%group_by(dec)%>%summarise(max(adjmou)))[[2]]
dat49$varnames<-rep("adjmou",nrow(dat49))
...
dat50$varnames<-rep("totrev",nrow(dat50))
...
dat51$varnames<-rep("adjrev",nrow(dat51))
...
dat52$varnames<-rep("avgrev",nrow(dat52))

#<53> Variable "comp_dat_Mean" ===>***Data transfermation then omit****
...
dat55$varnames<-rep("plcd_attempt_Mean",nrow(dat55))
...
dat56$varnames<-rep("complete_Mean",nrow(dat56))

#Adding all appropriate dat1 to dat54 objects to create a dat object
dat<-rbind(dat4,dat5,dat6,dat7,dat8,dat9,dat10,dat11,dat12,dat14,dat15,dat18,dat19,dat20,dat21,dat22,dat23,dat24,dat25,dat26,dat28,dat29,dat30,dat33,dat43,dat44,dat48,dat49,dat50,dat51,dat52,dat55,dat56)


#Exporting Deciled Variables
write.csv(dat,"Deciled Usable Continous variables.csv", row.names = F)

#Remove Variables that could not be deciled as will come insignificant in the model
#Also omitting transformed vars "comp_vce_Mean", "comp_dat_Mean", plcd_dat_Mean 
names(tele1)
...
#<19> Variable "crclscod" ======>****some levels show less than 5% churn rate. So Omit as well come insignificant ****
summary(tele$crclscod)
...
datC23$var.Name<-rep("area",nrow(datC23))

#<24> Variable "hnd_webcap"
summary(tele$hnd_webcap)
...
# use Vars as Factor => age2, models, actvsubs, uniqsubs, forgntv1, mtrcycle, truck,
...
#<36> Variable "forgntv1"
summary(tele$forgntvl)
...
datC36$var.Name<-rep("forgntv1",nrow(datC36))

#<37> Variable "mtrcycle"
summary(tele$mtrcycle)
...
datC_1<-rbind(datC19,datC20,datC21,datC22,datC23,datC24,datC25,datC26,datC27,datC28,datC29)
...
#Factor Variables=> as1_flag, prizm_social_one, area, refurb_new, hnd_webcap,
#marital, ethnic, age1, age2, models, hnd_price, actvsubs, "uniqsubs",
#"forgntv1","mtrcycle", "truck",churn, car_buy, customer_ID, retdays_1
...
table(tele$churn)/nrow(tele1)

#Convert to factor and Create Dummay Variables =>
               #age1, age2, models, hnd_Price, actvsubs, uniqsubs, forgntv1,mtrcycle, truck, Customer ID, churn
str(tele1$age1)
tele1$age1_1<-ifelse(tele1$age1==0, "Defualt",ifelse(tele1$age1<=30,"Young",ifelse(tele1$age1>30 & tele1$age1<=55,"Mid Age","Old")))
str(tele1$age1_1)
...
train$hnd_price_79.98<-ifelse(train$hnd_price == "79.9899902", 1, 0)
...
test$hnd_price_105.08<-ifelse(test$hnd_price == "105.08038078331", 1, 0)

train$hnd_price_129.9899902<-ifelse(train$hnd_price == "129.9899902", 1, 0)
test$hnd_price_129.9899902<-ifelse(test$hnd_price == "129.9899902", 1, 0)

train$hnd_price_149.98<-ifelse(train$hnd_price == "149.9899902", 1, 0)
test$hnd_price_149.98<-ifelse(test$hnd_price == "149.9899902", 1, 0)

train$hnd_price_199.98<-ifelse(train$hnd_price == "199.98999902", 1, 0)
...
## *** Rerunnung model with significant Variable **** ##
names(train)
mod1<-glm(churn ~ mou_Mean + totmrc_Mean + rev_Range + mou_Range + change_mou + drop_blk_Mean + drop_vce_Range + mou_opkv_Range + months + eqpdays + iwylis_vce_Mean + ovrrev_Mean + avgmou + avg3qty + avgqty + avg6mou + asl_flag_Y + area_Cal_Nrth + area_texas + area_nrthflrda + area_nrthwst + area_southflrda + area_southwst + area_tenese + refurb_R + ethnic_C + ethnic_N + ethnic_O + ethnic_S + ethnic_U + ethnic_Z + hnd_price_79.98 + hnd_price_105.08 + hnd_price_149.98 + hnd_price_199.98 + hnd_price_249.98 + unq_2 + unq_3 + unq_4 + unq_5 + unq_6 + unq_7 + unq_9 + truck + adjmou + totrev + retdays_1 + complete_Mean + przm_social_R + przm_social_T + age1_Mid_Age + age1_Old + age1_Young,data=train,family="binomial")
summary(mod1)

## *** Furthur Rerunning Model with significant Variables **** ##
mod2<-glm(churn ~ mou_Mean + totmrc_Mean + rev_Range + mou_Range + change_mou + drop_blk_Mean + drop_vce_Range + mou_opkv_Range + months + eqpdays + iwylis_vce_Mean + ovrrev_Mean + avgmou + avg3qty + avgqty + avg6mou + asl_flag_Y + area_Cal_Nrth + area_texas + area_nrthflrda + area_nrthwst + area_southflrda + area_southwst + area_tenese + refurb_R + ethnic_C + ethnic_N + ethnic_O + ethnic_S + ethnic_U + ethnic_Z + hnd_price_105.08 + hnd_price_249.98 + unq_2 + unq_3 + unq_4 + unq_7 + adjmou + totrev + retdays_1 + complete_Mean +przm_social_R + przm_social_T + age1_Old + age1_Young, data=train,family="binomial")
summary(mod2)
...
#Re-running the model with above vars omitted to remove problem of Multicollinearity.
mod3<-glm(churn ~ totmrc_Mean + rev_Range + mou_Range + change_mou + drop_blk_Mean + drop_vce_Range + mou_opkv_Range + months + eqpdays + iwylis_vce_Mean + ovrrev_Mean + avgqty + asl_flag_Y + area_Cal_Nrth + area_texas + area_nrthflrda + area_nrthwst + area_southflrda + area_southwst + area_tenese + refurb_R + ethnic_C + ethnic_N + ethnic_O + ethnic_S + ethnic_U + ethnic_Z + hnd_price_79.98 + hnd_price_105.08 + hnd_price_129.9899902 + hnd_price_149.98 + hnd_price_199.98 + hnd_price_249.98 + unq_2 + unq_3 + unq_4 + unq_7 + adjmou + totrev + retdays_1 + complete_Mean + przm_social_R + przm_social_T + age1_Old + age1_Young, data=train, family="binomial")
summary(mod3)

#Removing var mou_Mean
mod4<-glm(churn ~ totmrc_Mean + rev_Range + mou_Range + change_mou + drop_blk_Mean + drop_vce_Range + mou_opkv_Range + months + eqpdays + iwylis_vce_Mean + ovrrev_Mean + avgqty + asl_flag_Y + area_Cal_Nrth + area_texas + area_nrthflrda + area_nrthwst + area_southflrda + area_southwst + area_tenese + refurb_R + ethnic_C + ethnic_N + ethnic_O + ethnic_S + ethnic_U + ethnic_Z + hnd_price_105.08 + hnd_price_129.9899902 + hnd_price_249.98 + unq_2 + unq_3 + unq_4 + unq_7 + totrev + retdays_1 + complete_Mean + przm_social_R + przm_social_T  + age1_Old + age1_Young, data=train, family="binomial")
summary(mod4)
...
mod5 <- glm(churn ~ totmrc_Mean + rev_Range + mou_Range + change_mou + drop_blk_Mean + drop_vce_Range + 
              ...
              ethnic_Z + hnd_price_79.98 + hnd_price_105.08 + hnd_price_149.98 + hnd_price_199.98 + 
              hnd_price_249.98 + unq_2 + unq_3 + unq_4 + unq_7 + adjmou + totrev + retdays_1 + complete_Mean +
              przm_social_R + przm_social_T + age1_Old + age1_Young, data = train, family ="binomial")
summary(mod5)
...
mod6 <- glm(churn ~ totmrc_Mean + rev_Range + mou_Range + change_mou + drop_blk_Mean + drop_vce_Range + 
              ...
              area_southwst + area_tenese + refurb_R + ethnic_C + ethnic_N + ethnic_S + ethnic_O + ethnic_U +
              ethnic_Z + hnd_price_105.08 + hnd_price_249.98 + unq_2 + unq_3 + unq_4 + unq_7 + adjmou + totrev + retdays_1 + complete_Mean +
              przm_social_R + przm_social_T + age1_Mid_Age, data=train,family="binomial")
summary(mod6)
...
#Preicted  Values ==> Preicting the probability of a customer churning.
pred<-predict(mod6, type="response", newdata = test)
head(pred)

#Assuming cut-off Probability as per the churn rate in data set
table(tele1$churn)/nrow(tele1)


#choosing cutoff value accoring to kappa value
#s<-seq(0.2,0.5,0.01)
#n<-1
...
#We can this probability to extract the data of customers who are highly likely to churn.


## *** Answering Business Quetions ****##
## Top Line Quetions of Interest to senior Management:
...
# The above library is not avilable to me on LMS so i am manually looking at the beta coefficients
head(sort(abs(mod6$coefficients),decreasing = T),10)
summary(mod6)

#from the summary of my final model, "Mod6".
## The model results show that the top5 factors affecting churn are:
### a. unq_7                   with beta coefficient of 0.735625538
### b. retdays_1               with beta coefficient of 0.670774312
### c. ethnic_o                with beat coefficient of 0.313047896
...
# to customers who makes retention calls, at the earliest as per thier grieviances. Special plans should be rolled out for
# people with Asian Ethnicity. Special plans should be rolled out for customers located in NORTHWEST/ROCKY
...
# variables totmrc_Mean i.e. 'base plan charge' reprresenting cost to customer,
# var rev_Range i.e. 'Range of Revenue(charge amount)' representing billing amout,
# var ovrrev_Mean = DATOVR_MEAN + VCEOVR_MEAN i.e. 'Mean overage revenue' (It is the sum of data and voice Overage revenues)
# representing the overage revenue earned from customers after billing the same to them.
...
# var ovrrev_Mean has beta values of 0.007265908 meaning a unit increase in this variable is causing 
...
# From the above statistics, data expalins the folllowing:


# Variable mou_Range i.e. with a unit increase in 'Range of number of use',
...
#  off-peak voice calls, there is a decrease in churn by -0.001117168units.
...
#  there is a decrease in churn by -0.00032749 units.
...
# Var retdays_1 representing values capture in the Variable retdays i.e. with unit increase in 
...
# Of the above variable, the beta coefficient of variable retdays_1 is expressing a very important
# factor influencing churn behaviour. That is with the increase in the number if days since a customer
...
# 2b) Are data usage connetivity issues turning out to costly? In other words, is it leading to churn?
...
# datovr_Range - Mean of revenue of data overage
# drop_dat_Mean - Mean no. of dropped / failed data calls

# The above variables express data usage connectivity.
quantile(tele$plcd_dat_Mean,probs=c(0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.82,0.84,0.85,0.90,1))
...
# have switched operators in recent months reported two key information sources in thier decision:
...
# and service tp provide maximum customer satisfaction and reduce churn.
# Since there is not enough usable data for the above variables they are not showing any influence 
# on the churn Behabiour at Mobicom.


# 3.Would you recommend rate plan migration as a proctive retention strategy?

# Variable ovrrv_Mean has beta coefficient of 0.007265908.
...
# The Beta coefficient is not showing a strong impact of overage billing as an influncer
# of churn behaviour.
...
# 4. What would be your recommendation on how to use this churn model for for prioritisation
...
pred4<-ifelse(pred4>=0.3040815, 1,0)
...
# Falling ARPU forcast is also a concern and therefore, Mobicom would like to save their high revenue
...
## and Target list can be extracted as follows:
...
write.csv(Targeted1, "High_Revenue_Target_Custoemers.csv",row.names = F)
